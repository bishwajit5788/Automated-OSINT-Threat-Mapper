...
Active scanning is limited to a caller-supplied list or a small common-port
profile. Use only against systems you are authorized to assess. No exploitation
is performed.
"""
...
def _history_save(domain: str, payload: ReconResponse) -> str:
    ...
    if not previous:
        return "baseline"
    return "unchanged" if previous[0] == fingerprint else "changed"
...
async def query_crt_sh(domain: str, *, demo_mode: bool = False) -> Tuple[List[Subdomain], str]:
    ...
    try:
        async with httpx.AsyncClient(timeout=12, follow_redirects=True) as client:
            response = await client.get(f"https://crt.sh/?q=%.{domain}&output=json", headers={"User-Agent":"AetherMap-OSINT/2.0"})
    ...
    try:
        ...
        try:
            if port in {80,3000,5000,8000,8080,8443,9000}:
                ...
                await writer.drain()
            return await asyncio.wait_for(reader.read(1024), timeout=1.2)
        finally:
            writer.close()
...
async def scan_tcp_services(ip: str, ports: List[int]) -> List[PortService]:
    ...
    for port, data in zip(ports, results):
        if data is not None:
            ...
            services.append(PortService(port=port, service_name=service, product=product, version=version, banner=banner))
    return services
...
def _cpe_candidates(product: str) -> List[str]:
    ...
    return mappings.get(next((key for key in mappings if key in p), ""), [])


async def correlate_nvd(service: PortService) -> PortService:
    candidates = _cpe_candidates(service.product)
    if not candidates or service.version == "Unknown": return service
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            cpe_ref = None
            for keyword in candidates:
                response = await client.get("https://services.nvd.nist.gov/rest/json/cpes/2.0", params={"keywordSearch":keyword,"resultsPerPage":20})
                if response.status_code != 200: continue
                for item in response.json().get("products", []):
                    names = item.get("cpe", {}).get("cpeName", [])
                    if names:
                        cpe = names[0].get("cpeName")
                        if cpe and cpe.startswith("cpe:2.3:a:"):
                            cpe_ref = cpe; break
                if cpe_ref: break
            if not cpe_ref: return service
            parts = cpe_ref.split(":")
            exact_cpe = cpe_ref
            if len(parts) >= 6:
                parts[5] = service.version
                exact_cpe = ":".join(parts)
            response = await client.get("https://services.nvd.nist.gov/rest/json/cves/2.0", params={"cpeName":exact_cpe,"resultsPerPage":50})
            if response.status_code != 200: return service
            vulns = []
            ...
            service.cpe, service.vulnerabilities = exact_cpe, vulns[:25]
...
async def orchestrate_recon(domain: str, *, demo_mode: bool = False, ports: Optional[List[int]] = None) -> ReconResponse:
    ...
    if demo_mode:
        ...
        services = await scan_tcp_services(ip,scan_ports) if dns_ok else []
        services = list(await asyncio.gather(*[correlate_nvd(s) for s in services])) if services else []
    for sub in subdomains:
        if sub.name == domain: sub.ip,sub.status = ip if dns_ok else "Unknown","resolved" if dns_ok else "unresolved"
    score,risk,summary = compute_threat_metrics(services,subdomains)
    response = ReconResponse(target_domain=domain,root_ip=ip,timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(),threat_score=score,risk_level=risk,subdomains=subdomains,services=services,vulnerability_summary=summary,metadata=ReconMetadata(execution_time_ms=round((time.perf_counter()-started)*1000,2),sources_queried=["DNS","Certificate Transparency (crt.sh)","TCP connect scanner","NVD CPE/CVE API"],dns_resolved=dns_ok,crt_sh_status=crt_status,network_intel_status="local TCP scanner",findings_mode="demo" if demo_mode else "passive-active",scan_ports=scan_ports,open_ports=len(services)))
